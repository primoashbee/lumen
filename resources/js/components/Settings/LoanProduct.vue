						...
						<div class="form-group p0 col-lg-6 mt-4 d-table-row">
							<div class="form-check tranch mx-w3 d-table-cell">
		                        <label class="form-check-label" for="has_tranches">
		                            <input class="form-check-input cb-type" id="has_tranches" type="checkbox" v-model="form.has_tranches" @click="checkedTranched($event)">
...
export default {
    ...
    methods: {
        ...
        getProduct(){
            axios.get('/settings/loan/product/edit/'+this.id)
                .then(res=>{
                    this.isLoading = false;
                    this.form = res.data.loan
                })
    ...
    computed : {
        postUrl(){
            return  '/settings/loan/edit/'+this.id
        }
    }
}
</script>
